...
- **三路融合的诚实边界**：官方 Anthropic SDK 无 embeddings 资源，向量路在默认配置下是本地字符 n-gram 哈希——词面近似而非语义检索；配置第三方 `base_url` 时该路整体禁用，权重切换为 0.85/0.15。该弱点在意图侧已由本地语义原型分类器解决（见已知边界），RAG 侧接入真实 embedding API 仍是明确的优化项。
...
- 意图基准的标注协议与评测触碰记录见 [docs/BENCHMARK.md](docs/BENCHMARK.md)；人工盲测工具包见 `benchmarks/intent/blind_test/`。
